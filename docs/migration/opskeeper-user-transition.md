...
> 解释兼容命令、数据映射和演进事实。`opskeeper`、`ops-keeper` 仅代表历史
> 标识、兼容命令或数据映射；当前产品是 **OpsKeeper**——AgentTeams 原生的
> 多 Agent 事故诊断与安全执行平台。对外支持入口以本仓库 README 为准。
...
| **D 类（潜在用户）** | 知道 ops-keeper 但未使用 | 不计 | **重新定位**：按历史迁移方案营销 |
...
- 营销统一口径：OpsKeeper 提供历史项目迁移能力（兼容 `ops-keeper` 数据）
