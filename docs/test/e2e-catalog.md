...
| # | 用例 | 触发 | 关键断言点 | 优 | 实现 |
|---|---|---|---|---|---|
| A1 | 首次 purge install | shell: `uninstall.sh --purge --purge-edge` → `OPSKEEPER_PUBLIC_URL=… install.sh` | `healthz=200`、容器全 up、`.env` 有 ADMIN_PASSWORD、`/v1/auth/login` 通 | **P0** | |
...
| A4 | install.sh 非交互 env 预置 | `OPSKEEPER_PUBLIC_URL=… curl \| bash` | 不阻塞、写入对的 PUBLIC_URL | P1 | |
...
| A7 | manager 自观测指标 | 容器跑着 | Prom 抓到 `opskeeper_llm_*`、`opskeeper_alert_events_total`、`opskeeper_im_inbound_total` | P2 | |
...
| D2 | logs plugin → Loki | edge journald 写日志 | Loki `{device_id="N",opskeeper_source="journald"}` 有行 | **P0** | |
...
| F5 | 后台自动 fire 用 OPSKEEPER_DEFAULT_LOCALE | 设 env=zh fire incident | report 中文 | P1 | |
...
| H7 | /new 命令切新 session | 发 /new | `im_threads.opskeeper_session_id` 换、新 session | P2 | |
...
| # | 用例 | 触发 | 关键断言点 | 优 | 实现 |
|---|---|---|---|---|---|
| M1 | 开 ws → 发命令 → 回显 | WebSocket + `whoami` | 回显 opskeeper-edge | P1 | |
