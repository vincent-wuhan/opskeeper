...
// (HLD-017). AES-256-GCM, key derived from OPSKEEPER_SECRET_KEY. Mirrors
...
var (
	keyOnce sync.Once
	keyVal  [32]byte
	keyWeak bool // true when OPSKEEPER_SECRET_KEY was unset (derived fallback)
)

// loadKey derives the 32-byte AES key from OPSKEEPER_SECRET_KEY (sha256 of the
...
func loadKey() {
	keyOnce.Do(func() {
		env := strings.TrimSpace(os.Getenv("OPSKEEPER_SECRET_KEY"))
		if env == "" {
			keyWeak = true
			env = "opskeeper-insecure-default-secret-key-set-OPSKEEPER_SECRET_KEY"
		}
...
// fallback (OPSKEEPER_SECRET_KEY unset). main.go logs a warning when true.
func KeyIsWeak() bool {
	loadKey()
	return keyWeak
}
...
func Decrypt(enc string) (string, error) {
	...
	if err != nil {
		return "", fmt.Errorf("secretbox: open (wrong OPSKEEPER_SECRET_KEY?): %w", err)
	}
