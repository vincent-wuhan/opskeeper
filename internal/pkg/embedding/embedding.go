...
//
//	OPSKEEPER_EMBEDDING_PROVIDER  "openai" | "" (default openai)
//	OPSKEEPER_EMBEDDING_MODEL     model id, e.g. "text-embedding-3-small"
//	OPSKEEPER_EMBEDDING_BASE_URL  HTTP base; empty = api.openai.com
//	OPSKEEPER_EMBEDDING_API_KEY   bearer token
//	OPSKEEPER_EMBEDDING_DIM       expected dimensions (must match qdrant
//	                            collection)
package embedding
...
//	  cfg.BaseURL (interpreted as CacheDir) or OPSKEEPER_EMBEDDING_CACHE_DIR
//	  (default /var/lib/opskeeper/embeddings). cfg.Model picks the variant
...
func New(cfg Config) (Embedder, error) {
	...
	case "openai":
		e, err := newOpenAI(cfg)
		if err != nil {
			// typed-nil → interface(nil) so callers' `embedder != nil` works.
			return nil, err
		}
		return e, nil
...
func newOpenAI(cfg Config) (*openAIEmbedder, error) {
	...
	if apiKey == "" {
		return nil, errors.New("embedding: api_key required (set OPSKEEPER_EMBEDDING_API_KEY)")
	}
	log := cfg.Log
	if log == nil {
		log = slog.Default()
