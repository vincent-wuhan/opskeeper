...
//
// Triggered when OPSKEEPER_EMBEDDING_PROVIDER=local (or "fastembed" /
...
// location comes from OPSKEEPER_EMBEDDING_CACHE_DIR (default
// /var/lib/opskeeper/embeddings).
func newLocal(cfg Config) (*localEmbedder, error) {
	model, err := resolveModel(cfg.Model)
	if err != nil {
		return nil, err
	}
	cacheDir := strings.TrimSpace(os.Getenv("OPSKEEPER_EMBEDDING_CACHE_DIR"))
	if cacheDir == "" {
		cacheDir = "/var/lib/opskeeper/embeddings"
	}
	...
	// If the operator set OPSKEEPER_EMBEDDING_DIM explicitly, sanity check
	...
	if cfg.Dim > 0 && cfg.Dim != dim {
		return nil, fmt.Errorf("embedding: OPSKEEPER_EMBEDDING_DIM=%d doesn't match model %s dim %d",
			cfg.Dim, model, dim)
	}
	showProgress := false
