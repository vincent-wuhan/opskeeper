...
// the X-Scope-OrgID header is hardcoded to "opskeeper" (matches what nginx
...
func (c *Client) do(ctx context.Context, path string, q url.Values) ([]byte, error) {
	...
	req.Header.Set("User-Agent", "opskeeper-logquery/0.1")
	// Single-tenant install — nginx injects X-Scope-OrgID: opskeeper on
	// the ingest side, so use the same on read.
	req.Header.Set("X-Scope-OrgID", "opskeeper")
