...
func TestMetricsIncrementOnSuccess(t *testing.T) {
	...
	if got := counterValue(t, reg, "opskeeper_llm_requests_total", "gpt-4o", "success"); got != 1 {
		t.Errorf("success counter = %v, want 1", got)
	}
	if got := counterValue(t, reg, "opskeeper_llm_tokens_total", "gpt-4o", "prompt"); got != 42 {
		t.Errorf("prompt tokens counter = %v, want 42", got)
	}
	if got := counterValue(t, reg, "opskeeper_llm_tokens_total", "gpt-4o", "completion"); got != 8 {
