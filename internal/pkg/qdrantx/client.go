...
// SetPayloadByFilter updates payload fields on every point matching the
// supplied must-clauses. It is used for safe, idempotent visibility
// migrations without re-embedding existing vectors.
func (c *Client) SetPayloadByFilter(
	ctx context.Context,
	collection string,
	...
	if len(payload) == 0 {
		return errors.New("qdrant: SetPayloadByFilter requires payload")
	}
	filter := buildFilter(mustMatch)
	if filter == nil {
		return errors.New("qdrant: SetPayloadByFilter requires at least one match clause")
	}
	body := map[string]any{
		"payload": payload,
		"filter":  filter,
	...
	resp, err := c.do(ctx, http.MethodPost, "/collections/"+collection+"/points/payload", body)
	if err != nil {
		return err
	}
	defer resp.Body.Close()
	if resp.StatusCode/100 != 2 {
		raw, _ := io.ReadAll(resp.Body)
		return fmt.Errorf("qdrant: set payload: http %d: %s", resp.StatusCode, truncate(string(raw), 256))
	}
	return nil
}
