...
// then set OpskeeperSessionID; we don't do that here because the
...
// RotateThreadSession overwrites opskeeper_session_id on an existing
...
func (r *Repo) RotateThreadSession(ctx context.Context, threadID uint64, newSessionID string) error {
	return r.db.WithContext(ctx).Model(&model.ImThread{}).
		...
		Updates(map[string]any{
			"opskeeper_session_id": newSessionID,
