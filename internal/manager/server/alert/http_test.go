...
type fakeWebhookService struct {
	input svc.AlertmanagerWebhookInput
	err   error
}

func (f *fakeWebhookService) IngestAlertmanager(_ context.Context, _ svc.Caller, in svc.AlertmanagerWebhookInput) (*svc.AlertmanagerIngestResult, error) {
	f.input = in
	if f.err != nil {
		return nil, f.err
	}
...
func TestAlertmanagerWebhookAuthAndDecode(t *testing.T) {
	service := &fakeWebhookService{}
	handler := NewHandler(nil, nil, nil).WithWebhookService(service).WithWebhookToken("secret")
	router := chi.NewRouter()
	handler.RegisterPublic(router)

	body := `{"alerts":[{"status":"firing","labels":{"alertname":"HostDown"},"fingerprint":"abc"}]}`
	...
	req = httptest.NewRequest(http.MethodPost, "/v1/alerts/webhook", strings.NewReader(`{`))
	...
	req = httptest.NewRequest(http.MethodPost, "/v1/alerts/webhook", strings.NewReader(body+" trailing"))
	req.Header.Set("Authorization", "Bearer secret")
	recorder = httptest.NewRecorder()
	router.ServeHTTP(recorder, req)
	if recorder.Code != http.StatusBadRequest {
		t.Fatalf("trailing content status = %d; want 400", recorder.Code)
	}
	...
	if recorder.Code != http.StatusOK {
		t.Fatalf("valid status = %d; want 200", recorder.Code)
	}
	if len(service.input.Alerts) != 1 || service.input.Alerts[0].Labels["alertname"] != "HostDown" {
