...
type WebhookService interface {
	IngestAlertmanager(ctx context.Context, caller svc.Caller, in svc.AlertmanagerWebhookInput) (*svc.AlertmanagerIngestResult, error)
}
...
func (h *Handler) WithWebhookService(service WebhookService) *Handler {
	h.webhooks = service
	return h
}

func (h *Handler) WithWebhookToken(token string) *Handler {
	h.webhookToken = token
	return h
}
...
func (h *Handler) RegisterPublic(r chi.Router) {
	r.Post("/v1/alerts/webhook", h.ingestAlertmanager)
}
...
type alertmanagerWebhookPayload struct {
	Alerts []svc.AlertmanagerAlert `json:"alerts"`
}
	...
	if h.investigations == nil {
		writeJSON(w, http.StatusOK, map[string]any{
			...
			"status_reason": "investigator not wired on this manager " +
				"(set OPSKEEPER_INVESTIGATOR_ENABLED=true and configure an LLM)",
		})
		return
	}
...
func (h *Handler) triggerIncidentInvestigation(w http.ResponseWriter, r *http.Request) {
	...
	if h.investigationsTrigger == nil {
		writeJSON(w, http.StatusServiceUnavailable, map[string]any{
			"incident_id":   id,
			"status":        "feature_disabled",
			"status_reason": "investigator not wired on this manager (set OPSKEEPER_INVESTIGATOR_ENABLED=true and configure an LLM)",
		})
		return
	}
...
func (h *Handler) ingestAlertmanager(w http.ResponseWriter, r *http.Request) {
	if h.webhooks == nil || h.webhookToken == "" {
		writeErr(w, errs.ErrNotWiredYet)
		return
	}
	token := strings.TrimSpace(strings.TrimPrefix(r.Header.Get("Authorization"), "Bearer "))
	if token == "" || subtle.ConstantTimeCompare([]byte(token), []byte(h.webhookToken)) != 1 {
		writeErr(w, errs.ErrUnauthorized)
		return
	}
	...
	result, err := h.webhooks.IngestAlertmanager(r.Context(), svc.Caller{}, svc.AlertmanagerWebhookInput{Alerts: payload.Alerts})
	if err != nil {
		writeErr(w, err)
		return
	}
...
// (OPSKEEPER_ALERT_EVAL_INTERVAL / OPSKEEPER_ALERT_COOLDOWN) — they're per-
