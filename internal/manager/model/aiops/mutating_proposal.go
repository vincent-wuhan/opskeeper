...
type MutatingProposal struct {
	...
	Decision string `gorm:"size:16;not null;default:pending;check:decision IN ('pending','approve','reject','expired','rolled_back');column:decision"`
	...
	// SeverityTier is one of SeveritySafe / SeverityMutating /
	// SeverityDangerous. Defaults to "mutating" for backwards compat.
	SeverityTier string `gorm:"size:16;not null;default:'mutating';column:severity_tier"`

	// TTLSeconds is the proposal lifetime. After this many seconds in
	// the pending state, Expirer auto-declines with DecisionExpired.
	// Zero = use DefaultTTLSeconds.
	TTLSeconds int `gorm:"not null;default:0;column:ttl_seconds"`

	// ExpiredAt is stamped when Expirer transitions to DecisionExpired.
	ExpiredAt *time.Time `gorm:"column:expired_at"`

	// RolledBackAt is stamped when operator marks the proposal rolled back.
	RolledBackAt *time.Time `gorm:"column:rolled_back_at"`

	// RollbackOf references the original proposal ID when this row is
	// itself a rollback proposal.
	RollbackOf *string `gorm:"type:char(36);column:rollback_of"`

	// CreatedAt is the intercept time (decorator entered Run);
	// DecidedAt is when the reviewer returned.
	CreatedAt  time.Time
	DecidedAt  *time.Time `gorm:"column:decided_at"`
	ExecutedAt *time.Time `gorm:"column:executed_at"`
}

// EffectiveTTL returns the proposal's TTL in seconds, falling back to
// DefaultTTLSeconds when TTLSeconds is unset (0).
func (p *MutatingProposal) EffectiveTTL() int {
	if p.TTLSeconds <= 0 {
...
// ValidateTransition checks the proposed decision is reachable from
// the current state. Returns nil on legal transitions, error otherwise.
//
// Allowed transitions:
//
//	pending   → approve | reject | expired
//	approve   → rolled_back (after execution)
//	reject    → (terminal)
//	expired   → (terminal)
//	rolled_back → (terminal)
func (p *MutatingProposal) ValidateTransition(next string) error {
	cur := p.Decision
	if cur == "" {
		cur = DecisionPending
	}
	...
	case DecisionPending:
		switch next {
		case DecisionApprove, DecisionReject, DecisionExpired:
			return nil
		}
	case DecisionApprove:
		if next == DecisionRolledBack {
			return nil
		}
	case DecisionReject, DecisionExpired, DecisionRolledBack:
		// No transitions out of terminal states.
	}
	return fmtErrTransition(cur, next)
}

func fmtErrTransition(from, to string) error {
...
func (e errTransition) Error() string {
	return "mutating_proposal: illegal transition " + e.from + " → " + e.to
}
...
const (
	...
	// DecisionExpired — pending proposal exceeded its TTL before
	// reviewer decided. Set by proposal.Expirer; the tool is NOT
	// dispatched. Surfaces to the operator as a soft auto-decline.
	DecisionExpired = "expired"
	// DecisionRolledBack — executed proposal was later rolled back
	// (operator ran the inverse SOP or used a rollback link). The
	// original execution row stays in the audit chain; this state
	// marks the post-hoc reversal.
...
const (
	...
	SeveritySafe = "safe"
	// SeverityMutating — single-step state change; operator must
	// confirm in the SPA inbox before execution.
	SeverityMutating = "mutating"
	// SeverityDangerous — irreversible or high-blast-radius; requires
	// two-person sign-off (reviewer approve + second approver).
	SeverityDangerous = "dangerous"
)

// DefaultTTLSeconds is the default proposal lifetime before the
// expirer auto-declines. Configurable per proposal via TTLSeconds.
