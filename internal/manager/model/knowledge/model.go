...
type Doc struct {
	ID         uint64
	SourceType string
	// TenantScopes is the Qdrant visibility set. Builtin-vault docs use
	// "global"; organization docs use "tenant:<id>". A signed tenant query
	// always intersects this set with global plus its own scope.
	TenantScopes []string
	RepoID       *uint64
	URL          string
...
type SearchOptions struct {
	...
	Limit      int
	// TenantID is required for MCP query_knowledge. It scopes tenant docs
	// while retaining the globally readable builtin vault.
	TenantID string
	// IncidentID opts this search into hybrid RRF recall auditing.
	IncidentID string
	// DatabaseType and FaultFingerprint retrieve structured runbook memory.
	DatabaseType     string
