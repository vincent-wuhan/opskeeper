...
func (f *fakeVec) Search(_ context.Context, _ string, _ []float32, opts qdrantx.SearchOpts) ([]qdrantx.SearchHit, error) {
...
func uploadHead(url string) qdrantx.SearchHit {
	const tenantScope = "tenant:42"
	id := uploadChunkDocID(tenantScope, url, 0)
	return qdrantx.SearchHit{
		ID: id,
		Payload: map[string]any{
			"source_type":   model.SourceUpload,
			"tenant_scopes": []any{tenantScope},
			"title":         "技术行业为什么总在发明新词",
			"content":       "old body",
			"url":           url,
...
func TestUpdateManualDoc_UploadDoc(t *testing.T) {
	...
	wantID := uploadChunkDocID("tenant:42", url, 0)
	if got.ID != wantID {
		t.Fatalf("head id changed: got %d want %d", got.ID, wantID)
	}
