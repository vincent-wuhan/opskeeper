# opskeeper-removed vault

The platform-vendor knowledge base shipped with OpsKeeper. The manager
auto-registers this tree at boot under the `opskeeper-removed` org so its contents
are available to RAG retrieval, agent skills (`query_knowledge`), and the
Knowledge UI out of the box.

This repo will live at **`github.com/builtin://vault`** (private). The
OpsKeeper manager clones it on first run and re-syncs periodically. Operators
...
1. Ensures the `opskeeper-removed` org exists
2. Ensures a `knowledge_repos` row points at this vault, owner_org = `opskeeper-removed`, kind = `builtin`
