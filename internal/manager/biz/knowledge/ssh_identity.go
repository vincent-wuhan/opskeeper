...
// Comment is fixed to "opskeeper-deploy" so a quick `ssh-keygen -lf`
...
func (u *Usecase) GenerateSSHIdentity(ctx context.Context, in GenerateSSHIdentityInput) (*model.SSHIdentity, error) {
	...
	block, err := ssh.MarshalPrivateKey(priv, "opskeeper-deploy")
	if err != nil {
	...
	// (e.g. `ssh-ed25519 AAAA... opskeeper-deploy`). This is exactly what
	// gets pasted into the host's Deploy keys page.
	sshPub, err := ssh.NewPublicKey(pub)
	if err != nil {
		return nil, fmt.Errorf("derive ssh public: %w", err)
	}
	pubLine := strings.TrimSpace(string(ssh.MarshalAuthorizedKey(sshPub))) + " opskeeper-deploy"
