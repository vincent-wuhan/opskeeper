...
func newTestUC(t *testing.T) (*Usecase, *fakeRepo, *fakeSkillReg, *fakeAgentReg, string, string) {
	...
	uc := NewUsecase(repo, sk, ag, Config{
		...
		StagingDir:       staging,
		AllowedSources:   []string{"local", "opskeeper-official"},
		DevMode:          false,
	}, nil)
...
func TestInstall_SourceAllowlist(t *testing.T) {
	uc, _, _, _, _, tmp := newTestUC(t)
	uc.cfg.AllowedSources = []string{"opskeeper-official"} // local NOT in list
...
// Used by the "opskeeper-official + verified" fixture below.
...
// (rather than "opskeeper-official") because SourceTypeRegistry fetch
...
func TestInstall_UnsignedSourceNotGated(t *testing.T) {
	uc, repo, _, _, _, tmp := newTestUC(t)
	uc.cfg.RequireSignedSources = []string{"opskeeper-official"} // local NOT gated
	uc.cfg.DevMode = false
