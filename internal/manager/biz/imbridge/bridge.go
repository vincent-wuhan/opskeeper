...
// + decrypted them; this layer maps the IM thread to an opskeeper
...
type Bridge struct {
	repo  Repo
	agent AgentSession
	// ServiceUserID is the opskeeper users.id used as owner_user_id for
...
// HandleInbound resolves the IM thread → opskeeper session and kicks off
...
//     same mapping row points at the same opskeeper session forever
//     unless a user explicitly resets.
//   - Reset: any user sends "/new" / "新会话" / "新建会话" / "重新开始"
//     — the bridge allocates a fresh opskeeper session, rotates the
...
func (b *Bridge) HandleInbound(ctx context.Context, sender Sender, msg InboundMessage) error {
	...
	case thread == nil:
		...
		// fresh opskeeper session + write the mapping row.
	...
	if err := b.agent.StreamMessage(ctx, thread.OpskeeperSessionID, userContent, emit); err != nil {
...
// sessionLabel is the chat-history-page title for opskeeper-side
