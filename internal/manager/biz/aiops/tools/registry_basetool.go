...
// land. Operators can override via OPSKEEPER_TOOLBAG_DEFERRAL_THRESHOLD.
...
//
// Wiring contract for callers (cmd/opskeeper/main.go):
//
...
func (r *Registry) BuildBaseTools() *ToolBag {
	...
	// 17b: recovery.execute — closed-loop recovery BaseTool that wraps
	// the restart_service dispatcher with a mutating-proposal gate.
	// Class="write" (delegates to host_restart_service which is also
	// "write"); the ReviewGate decorator intercepts the call the same
	// way as restart_service, AND InvokableRun's inner audit check
	// rejects when no approved proposal exists for the incident_id.
	// Gated on the same triple as restart_service PLUS the
	// recoveryAuditRepo seam (set via SetRecoveryAuditRepo) — without
	// the seam there is no way to verify the proposal, so the tool is
	// silently omitted. The restart_service tool is constructed locally
	// so recovery.execute can dispatch to it via the same fakeCaller
