...
func (t *TenantBoundTool) InvokableRun(ctx context.Context, argsJSON string, opts ...basetool.InvokeOption) (string, error) {
	...
	if tenant.AgentTeams != nil && tenant.AgentTeams.TenantID != "" {
		uidStr = tenant.AgentTeams.TenantID
	}
	opts = append(opts,
	...
	tenantValue := any(tenant.UserID)
	if tenant.AgentTeams != nil && tenant.AgentTeams.TenantID != "" {
		tenantValue = tenant.AgentTeams.TenantID
	}
	merged, err := injectTenantID(argsJSON, tenantValue)
	if err != nil {
...
func injectTenantID(argsJSON string, tenantValue any) (string, error) {
	if argsJSON == "" {
		argsJSON = "{}"
	}
	...
	enc, err := json.Marshal(tenantValue)
	if err != nil {
		return "", err
	}
	m["tenant_id"] = enc
