...
type Deps struct {
	...
	// PauseCoordinator 是 P1-2 阶段 2 任务 2.2：biz/hitl 统一 Proposal 的
	// 工具执行前 PausePoint 入口。Nil-safe（与 ReviewSpawner 同语义）：
	// nil → 不安装 pause_point 装饰器；非 nil → 插入 review_gate 之前。
...
func Wrap(inner basetool.BaseTool, deps Deps) basetool.BaseTool {
	...
	// pause_point 插在 review_gate 之后、tenant_bind 之前（见 pause_point.go
	// 链位置说明）。WithPausePoint 自身对 coord=nil 退化为 noop。
	tool = WithPausePoint(tool, deps.PauseCoordinator)
