...
func TestTenantBind_UsesAgentTeamsTenant(t *testing.T) {
	inner := &fakeTool{
		name:   "tenant_tool",
		result: "ok",
		params: json.RawMessage(`{"type":"object","properties":{"tenant_id":{"type":"string"}}}`),
	}
	...
	if !strings.Contains(inner.gotArgs, `"tenant_id":"tenant-agentteams"`) {
		t.Fatalf("args = %q, want signed AgentTeams tenant", inner.gotArgs)
	}
}
...
func TestMetric_Increments(t *testing.T) {
	...
	got := mustCounter(t, reg, "opskeeper_tool_invocations_total", "metric_tool", "success")
	if got != 1 {
		t.Errorf("success counter = %f, want 1", got)
	}
	gotE := mustCounter(t, reg, "opskeeper_tool_invocations_total", "metric_tool", "error")
	if gotE != 1 {
		t.Errorf("error counter = %f, want 1", gotE)
	}
}

// mustCounter reads counter opskeeper_tool_invocations_total{name=..,result=..}
...
func TestChain_RunsAllInOrder(t *testing.T) {
	...
	if got := mustCounter(t, reg, "opskeeper_tool_invocations_total", "chain_tool", "success"); got != 1 {
