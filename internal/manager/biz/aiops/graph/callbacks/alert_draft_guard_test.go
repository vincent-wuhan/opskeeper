...
func TestAlertDraftGuard_HidesSampleSourceMentionsForUnscopedMetricDraft(t *testing.T) {
	...
	guard.OnEnd(context.Background(), toolRunInfo(draftConfigChangeToolName), &einotool.CallbackOutput{
		Response: `{"kind":"config_draft","draft_hash":"sha256:real","payload":{"rule":{"kind":"metric_raw","spec":{"expr":"max by (device_id, opskeeper_source) (mysql_global_status_threads_connected) > 75"}}}}`,
	})
	out := &einomodel.CallbackOutput{Message: &schema.Message{
		Role:    schema.Assistant,
...
func TestAlertDraftGuard_KeepsExplicitSourceMentionsForScopedMetricDraft(t *testing.T) {
	...
	guard.OnEnd(context.Background(), toolRunInfo(draftConfigChangeToolName), &einotool.CallbackOutput{
		Response: `{"kind":"config_draft","draft_hash":"sha256:real","payload":{"rule":{"kind":"metric_raw","spec":{"source_explicit":true,"expr":"mysql_global_status_threads_connected{opskeeper_source=\"db:mysql-test\"} > 75"}}}}`,
	})
	out := &einomodel.CallbackOutput{Message: &schema.Message{
		Role:    schema.Assistant,
