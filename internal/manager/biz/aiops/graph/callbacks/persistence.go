...
// that wire opskeeper cross-cutting concerns into the new compose.Graph
...
// increments opskeeper_persist_errors_total{kind=...} for any persist
...
func NewPersistenceHandler(deps PersistenceDeps) *PersistenceHandler {
	...
	if deps.Registerer != nil {
		...
		h.lossCounter = registerOrExisting(deps.Registerer, prometheus.NewCounterVec(
			prometheus.CounterOpts{
				Name: "opskeeper_chat_tool_response_loss_total",
				Help: "Total OpsKeeper AIOps tool responses lost (OnEnd never fired) and autohealed via stub.",
			},
