...
func getMetricsCollectors(reg prometheus.Registerer) *metricsCollectors {
	...
	tinv := prometheus.NewCounterVec(
		prometheus.CounterOpts{
			Name: "opskeeper_tool_invocations_total",
			Help: "Total OpsKeeper tool invocations, split by name and result.",
		},
		[]string{"name", "result"},
	)
	tdur := prometheus.NewHistogramVec(
		prometheus.HistogramOpts{
			Name:    "opskeeper_tool_duration_seconds",
			Help:    "OpsKeeper tool invocation duration in seconds.",
	...
	gi := prometheus.NewCounterVec(
		prometheus.CounterOpts{
			Name: "opskeeper_graph_iterations_total",
			Help: "Total OpsKeeper agent graph terminations, split by result (success | max_iterations | error).",
		},
		[]string{"result"},
	)
	ct := prometheus.NewCounterVec(
		prometheus.CounterOpts{
			Name: "opskeeper_graph_chat_turns_total",
			Help: "Total ChatModel turns observed during OpsKeeper agent graph runs.",
		},
		[]string{"result"},
	)
...
// opskeeper_tool_invocations_total at the per-call seam — this handler
