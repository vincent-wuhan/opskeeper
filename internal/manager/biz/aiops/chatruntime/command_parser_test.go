...
`)
	...
	// metadata.opskeeper.activation should mirror the top-level activation
	// so downstream consumers reading either key see keyword mode.
	if sk.Metadata.Opskeeper.Activation.Mode != "keyword" {
		t.Errorf("metadata.opskeeper.activation.mode = %q, want keyword", sk.Metadata.Opskeeper.Activation.Mode)
	}
	if len(sk.Tools) != 0 {
