...
// hint. opskeeper does NOT enforce it as a hard restriction, since claude
// command tools (Bash / Edit / ...) don't map 1:1 to opskeeper tool
// classes.
...
//     by opskeeper; the claude-side tool restriction is informational)
...
func ConvertCommandFile(path string) (*Skill, []LoadWarning, error) {
	...
	if len(fm.AllowedTools) > 0 {
		...
		b.WriteString("\n")
		b.WriteString(fmt.Sprintf("> 上游 (claude) 允许使用的工具: %s。OpsKeeper 不强制此清单，仅作提示。\n",
