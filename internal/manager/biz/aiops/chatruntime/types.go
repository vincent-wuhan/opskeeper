...
// (metadata.opskeeper.activation) and
...
// SKILL.md `metadata.requires` shape. opskeeper reads this verbatim per
type Requires struct {
	...
	// fills each slot; at exec opskeeper resolves the {{field}} templates from
	// that credential's fields. opskeeper stays semantics-agnostic — the skill
...
// OpskeeperExt is the opskeeper-private extension subtree under metadata.opskeeper.
// — these fields are ignored by openclaw / claude-code, so
// the same SKILL.md file runs on multiple platforms.
type OpskeeperExt struct {
	...
	// the other is for ergonomics). Loader prefers OpskeeperExt.Activation
	// when both are set, with a warning.
	Activation Activation `yaml:"activation" json:"activation"`

	// MinOpskeeperVersion gates loading on the running opskeeper build (e.g.
	// ">=0.7.30"). Failed gate produces a warning, not an error, in PR-2.
	MinOpskeeperVersion string `yaml:"min_opskeeper_version" json:"min_opskeeper_version"`
}

// SkillMetadata is the `metadata` frontmatter subtree common to both
// openclaw and claude-code SKILL.md files plus the opskeeper extension.
type SkillMetadata struct {
	...
	// Opskeeper is the opskeeper-private extension subtree.
	Opskeeper OpskeeperExt `yaml:"opskeeper" json:"opskeeper"`
}
...
type ToolDecl struct {
	...
	// (when_to_use). opskeeper follows the same split at tool granularity.
...
type Skill struct {
	...
	// shape). also allows it under metadata.opskeeper; the
	// opskeeper one wins when both are set.
	Activation Activation `yaml:"activation" json:"activation"`
	...
	// Metadata is the `metadata` subtree (os, requires, opskeeper.*).
	...
	// adding new fields doesn't break opskeeper loading. Map values are
...
type Agent struct {
	...
	// declare os / requires / opskeeper extensions too.
	Metadata SkillMetadata `yaml:"metadata" json:"metadata"`
...
// skills + agents + commands together. opskeeper recognizes both
