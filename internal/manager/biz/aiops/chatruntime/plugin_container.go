...
// opskeeper recognizes both: the former is treated as the canonical, the
// latter is treated as a superset whose opskeeper-irrelevant fields are
// preserved into UIMetadata["openclaw_legacy"].
//
// The struct is deliberately permissive — we model only the fields
// opskeeper currently consumes; any other field is captured into Extras
...
//
// opskeeper recognizes the openclaw and claude-plugin manifest forms
...
func LoadPluginContainer(dir string) (*LoadResult, error) {
	...
	if info, err := os.Stat(filepath.Join(dir, ".mcp.json")); err == nil && !info.IsDir() {
		warnings = append(warnings, LoadWarning{
			...
			Reason: ".mcp.json found; OpsKeeper does not run MCP servers yet",
		})
	}
...
func walkSkillsRoot(skillsDir, resolvedRoot string) ([]*Skill, []LoadWarning) {
	...
	walkErr := filepath.Walk(skillsDir, func(path string, info os.FileInfo, walkErr error) error {
		...
		// top-level nor metadata.opskeeper.activation set a mode.
		if sk.Activation.Mode == "" {
			sk.Activation.Mode = "always"
		}
		if sk.Metadata.Opskeeper.Activation.Mode == "" {
			sk.Metadata.Opskeeper.Activation.Mode = sk.Activation.Mode
		}
...
// warning per hook file plus one summary warning. opskeeper never executes
// hooks.
func scanHooksDir(hooksDir, resolvedRoot string) []LoadWarning {
	...
	warnings = append(warnings, LoadWarning{
		...
		Reason: "hooks/ subdir found; OpsKeeper does not run plugin hooks",
	})
	...
	for _, p := range dropped {
		warnings = append(warnings, LoadWarning{
			Path:   p,
			Code:   "hooks_dropped",
			Reason: "hook file ignored; OpsKeeper never executes plugin hooks",
		})
	}
	return warnings
}
...
// openclawLegacyKeys are the openclaw-only fields opskeeper stuffs into
...
func parsePluginManifest(raw []byte, kind ContainerKind) (*Pack, []LoadWarning, error) {
	...
	// so the SPA can render them, but opskeeper's runtime ignores them.
