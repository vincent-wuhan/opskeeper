...
func (rt *Runtime) SpawnWorker(ctx context.Context, req SpawnRequest) (*Worker, error) {
	...
	effectiveOwnerID := ownerUserID
	if effectiveOwnerID == 0 {
		effectiveOwnerID = req.OwnerUserID
	}
	...
	parent = withWorkerOwner(parent, effectiveOwnerID)
...
func withWorkerOwner(ctx context.Context, ownerUserID uint64) context.Context {
	if ownerUserID == 0 {
		return ctx
	}
	return tenantctx.With(ctx, tenantctx.Tenant{UserID: ownerUserID})
}

// CountWorkersByStatus returns the live worker counts split by status.
// Used by the ADR-026 self-obs sampler ticker (cmd/opskeeper/main.go) to
// keep opskeeper_chatruntime_worker_sessions{status="running|pending"}
...
func filterToolsForAgentRole(bag []basetool.BaseTool, agentDef *Agent, isCoordinator bool, viewerOnly bool) []basetool.BaseTool {
	...
	for _, t := range bag {
		...
		// opskeeper-device question). The multi-turn sweep showed that BACKFIRED with
