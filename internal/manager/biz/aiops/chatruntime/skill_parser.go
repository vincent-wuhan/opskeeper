...
//     schema additions don't break opskeeper loading.
...
func ParseSkillMd(path string) (*Skill, []LoadWarning, error) {
	...
	// Reconcile metadata.opskeeper.activation vs top-level activation.
	// — both are accepted; opskeeper one wins on conflict, with
	// a warning so the author knows.
	if sk.Metadata.Opskeeper.Activation.Mode != "" {
		if sk.Activation.Mode != "" && sk.Metadata.Opskeeper.Activation.Mode != sk.Activation.Mode {
			warnings = append(warnings, LoadWarning{
				Path:   path,
				Code:   "activation_conflict",
				Reason: "both top-level activation and metadata.opskeeper.activation are set; using metadata.opskeeper.activation",
			})
		}
		sk.Activation = sk.Metadata.Opskeeper.Activation
	}
