...
func TestCoordinatorToolRoutingUsesOpsKeeperIdentity(t *testing.T) {
	if !strings.Contains(coordinatorToolRouting, "OpsKeeper device/edge") {
		t.Fatalf("coordinator routing has wrong product identity: %q", coordinatorToolRouting)
	}
}

func TestComposeSystemPrompt_BaseOnly(t *testing.T) {
	got := ComposeSystemPrompt("you are opskeeper.", nil, nil)
	if got != "you are opskeeper.\n\n"+coordinatorToolRouting {
		t.Errorf("got %q", got)
	}
}
