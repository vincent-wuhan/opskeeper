...
func normalizeGuessedJournaldLogSelector(selector string) string {
	...
	out := []string{`opskeeper_source=~"journald(:.*)?"`}
	hasSource := false
...
func isKnownLogSelectorLabel(label string) bool {
	...
	case "detected_level", "device_id", "filename", "identifier", "level", "opskeeper_source", "service_name", "unit":
		return true
	default:
		return false
	}
