...
var promMetricSourceIdentityMatcherRE = regexp.MustCompile(`(?i)\b(opskeeper_source|device_id|job|instance|service)\s*(=|!=|=~|!~)\s*"([^"]+)"`)
var logLineFilterExprRE = regexp.MustCompile(`^\s*(\|=~|\|=|\|~|!=|!~)\s*(.+?)\s*$`)
var logLabelPrefixFilterRE = regexp.MustCompile(`(?i)^\s*(?:\(\?i\))?\s*(detected_level|device_id|filename|identifier|level|opskeeper_source|priority|service_name|severity|unit)\s*(=|!=|=~|!~)\s*"?([A-Za-z0-9_:/-]+(?:\.[A-Za-z0-9_:/-]+)*)"?\s*(?:\.\*)?(.*)$`)
