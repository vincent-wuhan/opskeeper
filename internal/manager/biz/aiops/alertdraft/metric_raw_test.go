...
func TestCompileDraftCombinesMetricRawExprOperatorThreshold(t *testing.T) {
	got, err := CompileDraft(CompileInput{
		...
		Rule: RuleConfigInput{
			Kind: "metric_raw",
			Spec: map[string]interface{}{
				"expr":      `sum by (device_id, opskeeper_source) (pg_stat_database_numbackends) / max by (device_id, opskeeper_source) (pg_settings_max_connections) * 100`,
				"operator":  ">",
				"threshold": 85,
			},
		},
	...
	expr, _ := got.Rule.Spec["expr"].(string)
	want := `(sum by (device_id, opskeeper_source) (pg_stat_database_numbackends) / max by (device_id, opskeeper_source) (pg_settings_max_connections) * 100) > 85`
	if expr != want {
