...
func TestRenderAlertPromptIncludesCrossDomainRCA(t *testing.T) {
	prompt := renderAlertPrompt(&alertmodel.Incident{ID: 9, Rule: "db_latency", Severity: "critical"}, "en")
	for _, want := range []string{
		"database middleware",
		"runtime symbol to commit+file:line",
		"knowledge/runbook corroboration",
		"within 2 minutes at P95",
		"do not run every domain mechanically",
	} {
...
func TestEnqueuePropagatesOwnerToWorker(t *testing.T) {
	...
	if spawner.calls[0].OwnerUserID != 42 {
		t.Fatalf("OwnerUserID = %d, want 42", spawner.calls[0].OwnerUserID)
	}
}
...
func TestRunObservesTerminalDurationStatus(t *testing.T) {
	tests := []struct {
		name    string
		spawner *fakeSpawner
		want    string
		...
		{
			name:    "failed",
			spawner: &fakeSpawner{err: errors.New("provider timeout")},
			want:    alertmodel.InvestigationStatusFailed,
		},
	}
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			observed := make(chan string, 1)
			uc := NewUsecase(&fakeRepo{}, test.spawner, nil, Config{Enabled: true}, nil).
				WithDurationObserver(func(status string, _ time.Duration) { observed <- status })
			uc.Enqueue(context.Background(), &alertmodel.Incident{ID: 77, Rule: "cpu", Severity: "critical"})
			select {
			case got := <-observed:
				if got != test.want {
			...
			case <-time.After(2 * time.Second):
				t.Fatal("duration status not observed")
			}
		})
	}
}
