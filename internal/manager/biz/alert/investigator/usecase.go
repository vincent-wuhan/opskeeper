...
type Config struct {
	...
	// (OPSKEEPER_INVESTIGATOR_ENABLED=true) — first-version default OFF
	...
	// OPSKEEPER_DEFAULT_LOCALE env at boot — see [[feedback_ai_output_locale]]:
...
type Usecase struct {
	...
	cfg              Config
	log              *slog.Logger
	durationObserver func(status string, elapsed time.Duration)
...
func NewUsecase(repo Repo, spawner WorkerSpawner, summarizer LLMSummarizer, cfg Config, log *slog.Logger) *Usecase {
	...
	uc := &Usecase{
		...
		durationObserver: observeInvestigationDuration,
		inflight:         map[string]bool{},
	}
	if cfg.MaxConcurrent > 0 {
...
// WithDurationObserver replaces the terminal-duration sink. Production uses
// the Prometheus observer installed by NewUsecase; tests can inject a
// deterministic callback without touching package-global collectors.
func (uc *Usecase) WithDurationObserver(observer func(status string, elapsed time.Duration)) *Usecase {
	uc.durationObserver = observer
	return uc
}

func observeInvestigationDuration(status string, elapsed time.Duration) {
	if managerprom.InvestigatorDuration != nil {
		managerprom.InvestigatorDuration.WithLabelValues(status).Observe(elapsed.Seconds())
	}
}
...
func (uc *Usecase) EnqueueWith(ctx context.Context, incident *alertmodel.Incident, opts EnqueueOpts) {
	...
	go uc.run(rep.ID, *incident, key, locale, ownerUserID)
}
...
func (uc *Usecase) run(reportID string, incident alertmodel.Incident, dedupKeyVal string, locale string, ownerUserID uint64) {
	start := time.Now()
	terminalStatus := alertmodel.InvestigationStatusFailed
	defer func() {
		if uc.durationObserver != nil {
			uc.durationObserver(terminalStatus, time.Since(start))
		}
		uc.inflightMu.Lock()
	...
	terminalStatus = alertmodel.InvestigationStatusReady
...
func renderAlertPrompt(in *alertmodel.Incident, locale string) string {
	...
	b.WriteString("Then follow the causal chain across relevant domains: host/observability, database middleware, runtime symbol to commit+file:line, and knowledge/runbook corroboration. ")
	b.WriteString("Only query a domain when the incident evidence points there; do not run every domain mechanically. End with a clear root-cause paragraph.\n")
	...
	b.WriteString("SLA: aim to finish the end-to-end RCA within 2 minutes at P95 by choosing high-information tools; this is a service objective, not permission to hide timeouts or omit available evidence.\n")
