...
func TestExtractStructured_ParsesValid(t *testing.T) {
  ...
  "evidence": [
    {"step": 1, "domain": "observability", "tool": "correlate_incident", "summary": "cpu spike + net spike"},
    {"step": 2, "domain": "middleware", "tool": "analyze_database_status", "summary": "slow query found"},
    {"step": 3, "domain": "git", "tool": "git.find_runtime_link", "summary": "query linked", "commit": "abc123", "repo": "orders", "file_path": "repo/order.go", "line_start": 42, "line_end": 45, "confidence": 0.91},
    {"step": 4, "domain": "knowledge", "tool": "query_knowledge", "summary": "known regression runbook"}
  ],
  ...
  "confidence": 0.85
	...
	if err := json.Unmarshal([]byte(fields.ConfidenceFactorsJSON), &factors); err != nil {
		t.Fatalf("confidence factors: %v", err)
	}
	if factors["cross_domain"] != true || factors["has_code_link"] != true || factors["has_knowledge_evidence"] != true {
		t.Fatalf("cross-domain factors not populated: %#v", factors)
	}
	domains, ok := factors["observed_domains"].([]any)
	if !ok || len(domains) != 4 {
...
func TestBuildConfidenceFactorsInfersLegacyEvidenceDomains(t *testing.T) {
	factorsJSON := buildConfidenceFactors(extractedReport{Evidence: []map[string]any{
		{"tool": "get_host_processes", "summary": "pid high"},
		{"tool": "analyze_database_status", "summary": "lock wait"},
		{"tool": "git.find_runtime_link", "commit": "abc", "file_path": "query.go"},
	}}, 3)
	var factors map[string]any
	if err := json.Unmarshal([]byte(factorsJSON), &factors); err != nil {
		t.Fatalf("Unmarshal: %v", err)
	}
	if factors["cross_domain"] != true || factors["has_code_link"] != true {
		t.Fatalf("legacy inference failed: %#v", factors)
	}
	if factors["has_knowledge_evidence"] != false {
		t.Fatalf("unexpected knowledge evidence: %#v", factors)
	}
}
