...
investigator wrote, output ONE compact JSON object with these keys:

  {
    ...
    "evidence":          [ {
      "step": 1,
      "domain": "host|middleware|change|topology|git|knowledge|observability",
      "tool": "...",
      "summary": "...",
      "commit": "<sha when code-linked>",
      "repo": "<repo when code-linked>",
      "file_path": "<path when code-linked>",
      "line_start": 1,
      "line_end": 1,
      "confidence": 0.0
    }, ... ],
...
- Set evidence.domain from the actual signal source. Preserve commit,
  repo, file_path, line_start, line_end, and confidence when a runtime
  symbol was linked to source. A knowledge/runbook match is domain=knowledge
  and must not be presented as live telemetry.
...
func buildConfidenceFactors(ex extractedReport, toolCallCount int) string {
	...
	for _, evidence := range ex.Evidence {
		tool, _ := evidence["tool"].(string)
		domain, _ := evidence["domain"].(string)
		domain = normalizeEvidenceDomain(domain)
		if domain == "" {
			domain = inferEvidenceDomain(tool)
		}
		if domain != "" {
			domains[domain] = struct{}{}
		}
		if domain == "knowledge" || tool == "query_knowledge" {
			hasKnowledgeEvidence = true
		}
		commit, _ := evidence["commit"].(string)
		filePath, _ := evidence["file_path"].(string)
		if strings.TrimSpace(commit) != "" && strings.TrimSpace(filePath) != "" {
			hasCodeLink = true
		}
	}
	observedDomains := make([]string, 0, len(domains))
	for domain := range domains {
		observedDomains = append(observedDomains, domain)
	}
	sort.Strings(observedDomains)
	factors := map[string]any{
		...
		"cross_domain":           len(observedDomains) > 1,
		"has_code_link":          hasCodeLink,
		"has_knowledge_evidence": hasKnowledgeEvidence,
	}
	b, _ := json.Marshal(factors)
	return string(b)
}

func normalizeEvidenceDomain(domain string) string {
	switch strings.ToLower(strings.TrimSpace(domain)) {
	case "host", "middleware", "change", "topology", "git", "knowledge", "observability":
...
func inferEvidenceDomain(tool string) string {
	tool = strings.ToLower(strings.TrimSpace(tool))
	switch {
	case tool == "query_knowledge":
		return "knowledge"
	case strings.HasPrefix(tool, "git.") || tool == "grep_source" || tool == "read_source" || tool == "list_repo_sources":
		return "git"
	case strings.HasPrefix(tool, "pg.") || strings.HasPrefix(tool, "redis.") || strings.HasPrefix(tool, "rabbitmq.") || strings.HasPrefix(tool, "kafka.") || tool == "analyze_database_status" || tool == "list_database_sources":
		return "middleware"
	case tool == "query_change_events":
		return "change"
	case tool == "expand_topology" || tool == "find_topology_node" || tool == "get_topology":
		return "topology"
	case strings.HasPrefix(tool, "get_host_") || strings.HasPrefix(tool, "host_"):
		return "host"
	case tool == "correlate_incident" || strings.HasPrefix(tool, "query_prom") || strings.HasPrefix(tool, "query_log") || strings.HasPrefix(tool, "query_trace"):
