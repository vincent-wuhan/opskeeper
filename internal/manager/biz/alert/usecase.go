...
// SchedulerMissedRun is the alert-facing representation of an overdue flow
// schedule. It deliberately avoids importing the scheduler domain package.
type SchedulerMissedRun struct {
	FlowID            string
	NodeID            string
	CronSpec          string
...
// RecordSchedulerMissedRun writes a warning incident through the normal alert
// lifecycle. The flow/node dedupe key makes repeated boot scans idempotent.
func (u *Usecase) RecordSchedulerMissedRun(ctx context.Context, source string, missed SchedulerMissedRun) error {
	if u == nil || u.repo == nil {
		return errs.ErrNotWiredYet
	}
	if source == "" {
		source = "scheduler-missed-run"
	}
	dedupeKey := fmt.Sprintf("%s:%s:%s", source, missed.FlowID, missed.NodeID)
	_, err := u.RecordFiring(ctx, FiringInput{
		ScopeType:   "global",
		Scope:       "scheduler",
		Rule:        source,
		RuleName:    "Scheduler missed run",
		Severity:    string(notify.SeverityWarning),
		OccurredAt:  missed.ExpectedFireAt,
		DedupeKey:   dedupeKey,
		SourceType:  model.RuleSourceBuiltin,
		Title:       "Workflow schedule missed",
		Summary:     fmt.Sprintf("flow=%s node=%s missed cron execution", missed.FlowID, missed.NodeID),
		Description: fmt.Sprintf("Cron %q expected at %s was missed by %d seconds.", missed.CronSpec, missed.ExpectedFireAt.UTC().Format(time.RFC3339), missed.MissedDurationSec),
		Labels:      map[string]string{"flow_id": missed.FlowID, "node_id": missed.NodeID, "source": source},
		Annotations: map[string]string{"cron": missed.CronSpec},
		RunbookURL:  "docs/operations-manual.md#scheduler-reliability",
	})
	return err
}
...
type AlertmanagerWebhookInput struct {
