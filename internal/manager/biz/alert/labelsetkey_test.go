...
// The same alert subject reported with vs without the opskeeper_source
...
func TestLabelSetKey_IgnoresProvenanceLabels(t *testing.T) {
	withSrc := map[string]string{"device_id": "1", "mountpoint": "/", "opskeeper_source": "embedded"}
	without := map[string]string{"device_id": "1", "mountpoint": "/"}
	if labelSetKey(withSrc) != labelSetKey(without) {
		t.Errorf("opskeeper_source must not split the key: with=%q without=%q", labelSetKey(withSrc), labelSetKey(without))
	}
