...
// change into a single opskeeper-managed Grafana dashboard.
//
// Sync is one-way (opskeeper → Grafana). Edits made directly in Grafana are
// NOT pulled back; on the next mirror push from opskeeper they get
...
// mental model: "build it in opskeeper, view it in Grafana".
//
...
// core fleet panels). Used at boot so the opskeeper-monitor dashboard exists
