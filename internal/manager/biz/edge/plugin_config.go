...
// should push to. Implementation lives at the wiring site (cmd/opskeeper)
// because it composes OPSKEEPER_PUBLIC_URL + the per-plugin path AND
...
// SetNotifier injects the notifier post-construction. cmd/opskeeper wires
...
// drops the binaries into /usr/local/lib/opskeeper-edge), so they're
