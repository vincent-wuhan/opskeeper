...
func TestSetDatabaseMetricsWritesSecretAndStripsPassword(t *testing.T) {
	...
	if req.Path != "/var/lib/opskeeper-edge/secrets/pg-prod.dsn" {
...
func TestSetDatabaseMetrics_WhenSecretWriteFails_RollsBackConfig(t *testing.T) {
	oldSpec := `{"sources":[{"id":"old-redis","db_type":"redis","enabled":true,"connection":{"type":"managed","path":"/var/lib/opskeeper-edge/secrets/old-redis.dsn","secret_set":true}}]}`
	repo := newFakePluginConfigRepo()
	...
	if !deleteReq.Delete || deleteReq.Path != "/var/lib/opskeeper-edge/secrets/old-redis.dsn" {
...
func TestSetDatabaseMetricsDeletesRemovedSecret(t *testing.T) {
	oldSpec := `{"sources":[{"id":"pg-prod","db_type":"postgresql","enabled":true,"connection":{"type":"managed","path":"/var/lib/opskeeper-edge/secrets/pg-prod.dsn","secret_set":true}},{"id":"redis-prod","db_type":"redis","enabled":true,"connection":{"type":"managed","path":"/var/lib/opskeeper-edge/secrets/redis-prod.dsn","secret_set":true}}]}`
	repo := newFakePluginConfigRepo()
	repo.rows[model.PluginNameDatabaseMetrics] = &model.PluginConfig{
		ID:         23,
		EdgeID:     7,
	...
	if req.SourceID != "redis-prod" || req.Path != "/var/lib/opskeeper-edge/secrets/redis-prod.dsn" {
			...
			"sources": []interface{}{
				map[string]interface{}{
					...
					"credentials": map[string]interface{}{
						...
						"tls_ca_file":     "/etc/opskeeper-edge/certs/ca.crt",
					},
				},
			},
		},
...
func TestSetDatabaseMetricsKeepsVisibleCredentialsAndPreservesPassword(t *testing.T) {
	...
	if req.DBType != "postgresql" || req.Path != "/var/lib/opskeeper-edge/secrets/pg-prod.dsn" {
		t.Fatalf("req = %#v", req)
	}
	...
	tests := []struct {
	...
	}{
		{
			...
			credentials: map[string]interface{}{
				...
				"tls_skip_verify": true,
				"tls_ca_file":     "/etc/opskeeper-edge/certs/ca.crt",
				"tls_cert_file":   "/etc/opskeeper-edge/certs/client.crt",
				"tls_key_file":    "/etc/opskeeper-edge/certs/client.key",
			},
			want: []string{
				"tls=skip-verify",
			},
		},
		{
			...
			want: []string{
				"sslmode=require",
				"sslrootcert=%2Fetc%2Fopskeeper-edge%2Fcerts%2Fca.crt",
				"sslcert=%2Fetc%2Fopskeeper-edge%2Fcerts%2Fclient.crt",
				"sslkey=%2Fetc%2Fopskeeper-edge%2Fcerts%2Fclient.key",
			},
		},
		{
			name:   "mongodb",
			...
			credentials: map[string]interface{}{
				...
				"tls_skip_verify": true,
				"tls_ca_file":     "/etc/opskeeper-edge/certs/ca.crt",
				"tls_cert_file":   "/etc/opskeeper-edge/certs/client.pem",
			},
			want: []string{
				"tls=true",
				"tlsInsecure=true",
...
func TestSetDatabaseMetricsKeepsAdvancedExporterOptionsForFourDBs(t *testing.T) {
			...
			"sources": []interface{}{
				...
				map[string]interface{}{
					...
					"exporter": map[string]interface{}{
						...
						"config_file":                             "/etc/opskeeper-edge/postgres_exporter.yml",
						"extend_query_path":                       "/etc/opskeeper-edge/postgres-queries.yaml",
	...
	if pgExporter["auto_discover_databases"] != true ||
		...
		pgExporter["config_file"] != "/etc/opskeeper-edge/postgres_exporter.yml" ||
		pgExporter["extend_query_path"] != "/etc/opskeeper-edge/postgres-queries.yaml" ||
