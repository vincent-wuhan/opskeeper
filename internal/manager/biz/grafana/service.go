...
//   - Sync: ensure the opskeeper folder, upsert the prom datasource, push
...
const (
	folderUID      = "opskeeper"
	folderTitle    = "opskeeper"
	datasourceUID  = "opskeeper-prometheus"
	datasourceName = "opskeeper-prometheus"
)
...
func New(settings *settingbiz.Service, tlsInsecure bool, log *slog.Logger) *Service {
	...
	return &Service{
		...
		panelDashboardUID:  "opskeeper-monitor",
		panelDashboardName: "opskeeper Monitor (managed)",
	}
}

// SetPanelDashboardUID overrides the Monitor mirror dashboard uid. cmd/
// opskeeper wires this from OPSKEEPER_GRAFANA_PANEL_DASHBOARD_UID at startup.
...
// opskeeper SA + token in the embedded Grafana so the operator doesn't
...
func (s *Service) BootstrapEmbedded(ctx context.Context, adminUser, adminPassword string) {
	...
	token, err := c.CreateServiceAccountToken(ctx, sa.ID, "opskeeper-bootstrap")
	if err != nil {
...
func (s *Service) Sync(ctx context.Context) (*SyncResult, error) {
	...
	// Forward the opskeeper-side prom credentials into Grafana's encrypted
...
// into a single opskeeper-managed Grafana dashboard. The dashboard's uid
// is fixed (panelDashboardUID, default "opskeeper-monitor") so re-pushes
...
func buildMonitorDashboardJSON(uid, title string, panels []*monitormodel.Panel) map[string]any {
	...
	return map[string]any{
		"uid":           uid,
		"title":         title,
		"description":   "Auto-managed by opskeeper. Edits in Grafana will be overwritten — change panels in the opskeeper Monitor page.",
		"tags":          []string{"opskeeper", "managed"},
		"editable":      true,
		"timezone":      "",
		"schemaVersion": 38,
		"panels":        gPanels,
...
// mapPanelType normalises opskeeper's panel type into Grafana's. Today
// they're 1:1, but we keep the indirection so future opskeeper types
