...
// ScheduleStateKey identifies one trigger.cron node.
type ScheduleStateKey struct {
	FlowID uint64
	NodeID string
}

// ScheduleStateRepo persists cron cursors independently from flow runs.
// Implementations live in data/flow and are injected into Scheduler.
type ScheduleStateRepo interface {
	LoadScheduleStates(ctx context.Context) ([]*model.FlowScheduleNextFire, error)
	UpsertScheduleState(ctx context.Context, state *model.FlowScheduleNextFire) error
	DeleteScheduleStatesNotIn(ctx context.Context, keys []ScheduleStateKey) error
}

// runRetention is how long finished runs are kept before PruneOldRuns
// reaps them. Overridable via OPSKEEPER_FLOW_RUN_RETENTION_DAYS.
...
func (u *Usecase) PruneOldRuns(ctx context.Context) {
	...
	if v := strings.TrimSpace(os.Getenv("OPSKEEPER_FLOW_RUN_RETENTION_DAYS")); v != "" {
