...
type AlertmanagerAlert = bizalert.AlertmanagerAlert
type AlertmanagerWebhookInput = bizalert.AlertmanagerWebhookInput
type AlertmanagerIngestResult = bizalert.AlertmanagerIngestResult
...
// from OPSKEEPER_NOTIFY_* env vars; PR-D unlocks UI-side CRUD.
...
func (s *Service) TestChannel(ctx context.Context, _ Caller, id uint64) (*ChannelTestResult, error) {
	...
	msg := notify.Message{
		Subject:    fmt.Sprintf("[opskeeper] 测试投递 — %s", channel.Name),
		Body:       "这是一条来自 opskeeper 通知渠道测试的消息，收到即视为渠道投递链路正常。",
	...
	// the global OPSKEEPER_NOTIFY_ENABLED master switch (an operator testing a
...
func (s *Service) IngestAlertmanager(ctx context.Context, _ Caller, in AlertmanagerWebhookInput) (*AlertmanagerIngestResult, error) {
	if s.uc == nil {
		return nil, errs.ErrNotWiredYet
	}
	return s.uc.IngestAlertmanager(ctx, in)
}
