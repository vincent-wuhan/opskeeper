// Package systemupgrade checks the upstream opskeeper release and prepares
...
const (
	defaultReleaseMetadataURL = "https://opskeeper.cloud/dl/latest.json"
	defaultGitHubReleaseURL   = "https://api.github.com/repos/vincent-wuhan/opskeeper/releases/latest"
	defaultDownloadBase       = "https://opskeeper.cloud/dl"
...
func (s *Service) fetchLatest(ctx context.Context, sourceURL string) (*releaseInfo, error) {
	...
	req.Header.Set("User-Agent", "opskeeper/"+strings.TrimSpace(s.cfg.CurrentVersion))
...
func buildArchCommand(version, base, arch string) string {
	pkg := fmt.Sprintf("opskeeper-%s-linux-%s", version, arch)
	return fmt.Sprintf(
