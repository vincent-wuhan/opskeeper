...
func makeBundle(t *testing.T, dir string) (tarballPath, outerSHA string) {
	...
	entries := []entry{
		{name: "opskeeper-edge", body: []byte("agent-v1"), mode: 0o755, dest: "/usr/local/bin/opskeeper-edge"},
		{name: "process_exporter", body: []byte("pe-v1"), mode: 0o755, dest: "/usr/local/lib/opskeeper-edge/process_exporter"},
	}
...
func TestExtractTarGz_HappyPath(t *testing.T) {
	...
	for _, name := range []string{"MANIFEST.txt", "opskeeper-edge", "process_exporter"} {
...
func TestVerifyManifest_DetectsTamper(t *testing.T) {
	...
	if err := os.WriteFile(filepath.Join(stage, "opskeeper-edge"), []byte("TAMPERED"), 0o755); err != nil {
		t.Fatal(err)
	}
