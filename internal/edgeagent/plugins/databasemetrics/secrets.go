...
const (
	secretBaseDir    = "/var/lib/opskeeper-edge/secrets"
...
// under /var/lib/opskeeper-edge/secrets so plugin config cannot turn this RPC
...
func stageManagedSecretWrite(plan managedSecretWritePlan) (stagedManagedSecretWrite, error) {
	...
	f, err := os.CreateTemp(dir, ".opskeeper-secret-*")
	if err != nil {
...
func reserveManagedSecretBackupPath(dir string) (string, error) {
	f, err := os.CreateTemp(dir, ".opskeeper-secret-backup-*")
	if err != nil {
