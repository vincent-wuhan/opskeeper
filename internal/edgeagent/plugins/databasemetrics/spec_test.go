...
func TestSourceCommandKeepsSecretsOutOfArgsWhereSupported(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			secretPath: "/etc/opskeeper-edge/secrets/mysql.my.cnf",
			secret:     "[client]\nuser=u\npassword=p",
			wantBinary: "/bin/mysqld_exporter",
			wantArgs:   []string{"--web.listen-address=127.0.0.1:19104", "--config.my-cnf=/etc/opskeeper-edge/secrets/mysql.my.cnf", "--tls.insecure-skip-verify"},
		},
		{
			dbType:     "postgresql",
			secretPath: "/etc/opskeeper-edge/secrets/pg.dsn",
		...
		{
			dbType:     "redis",
			tls:        tlsSpec{Enabled: true, SkipVerify: true, CAFile: "/etc/opskeeper-edge/certs/ca.crt", CertFile: "/etc/opskeeper-edge/certs/client.crt", KeyFile: "/etc/opskeeper-edge/certs/client.key"},
			secretPath: "/etc/opskeeper-edge/secrets/redis.dsn",
			secret:     "rediss://:p@127.0.0.1:6379",
			wantBinary: "/bin/redis_exporter",
			wantArgs:   []string{"--web.listen-address=127.0.0.1:19104", "-skip-tls-verification", "-tls-ca-cert-file=/etc/opskeeper-edge/certs/ca.crt", "-tls-client-cert-file=/etc/opskeeper-edge/certs/client.crt", "-tls-client-key-file=/etc/opskeeper-edge/certs/client.key"},
			wantEnv:    []string{"REDIS_ADDR=rediss://:p@127.0.0.1:6379"},
		},
		{
			dbType:     "mongodb",
			secretPath: "/etc/opskeeper-edge/secrets/mongo.dsn",
	...
	tests := []struct {
		...
		},
		{
			...
			wantArgs: []string{
				...
				"--config.file=/etc/opskeeper-edge/postgres_exporter.yml",
				"--extend.query-path=/etc/opskeeper-edge/postgres-queries.yaml",
				"--log.format=json",
				"--log.level=debug",
...
func TestParseSourceAcceptsManagedConnection(t *testing.T) {
	source, err := parseSource(0, map[string]interface{}{
		...
		"tls": map[string]interface{}{
			"enabled":     true,
			"skip_verify": true,
			"ca_file":     "/etc/opskeeper-edge/certs/ca.crt",
		},
		"connection": map[string]interface{}{
			"type": "managed",
			"path": "/var/lib/opskeeper-edge/secrets/mysql-prod.my.cnf",
		},
	})
	if err != nil {
	...
	if !source.TLS.Enabled || !source.TLS.SkipVerify || source.TLS.CAFile != "/etc/opskeeper-edge/certs/ca.crt" {
	...
	for _, tt := range tests {
		t.Run(tt.dbType, func(t *testing.T) {
			source, err := parseSource(0, map[string]interface{}{
				...
				"connection": map[string]interface{}{
					"type": "managed",
					"path": "/var/lib/opskeeper-edge/secrets/" + tt.dbType + "-prod.dsn",
				},
			})
			if err != nil {
				t.Fatalf("parseSource() error = %v", err)
...
func TestParseSourceUsesMongoDBExporterCollectors(t *testing.T) {
	source, err := parseSource(0, map[string]interface{}{
		...
		"connection": map[string]interface{}{
			"type": "managed",
			"path": "/var/lib/opskeeper-edge/secrets/mongo-prod.dsn",
		},
	...
	_, err := parseSpec(map[string]interface{}{
		"sources": []interface{}{
			...
			map[string]interface{}{
				...
				"connection": map[string]interface{}{
					"type": "managed",
					"path": "/var/lib/opskeeper-edge/secrets/mysql-copy.my.cnf",
				},
			},
		},
	})
