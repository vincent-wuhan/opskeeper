...
func TestBuildManagedSecretPreservingPasswordForPostgresSkipVerify(t *testing.T) {
	...
	current := "postgresql://opskeeper:old-secret@127.0.0.1:15432/metrics_test?sslmode=verify-full&sslrootcert=%2Fold%2Fca.crt\n"
	...
	if !strings.Contains(got, "postgresql://opskeeper:old-secret@127.0.0.1:15432/metrics_test") {
...
func TestBuildManagedSecretPreservingPasswordDoesNotLeakInvalidExistingURI(t *testing.T) {
	base := t.TempDir()
	path := filepath.Join(base, "pg-prod.dsn")
	current := "postgresql://opskeeper:old-secret@127.0.0.1:15432/%zz\n"
	if err := os.WriteFile(path, []byte(current), 0o600); err != nil {
		t.Fatalf("WriteFile() error = %v", err)
	}
