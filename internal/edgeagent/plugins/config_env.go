...
//
//	OPSKEEPER_EDGE_PLUGIN_<NAME>_ENABLED    = true|false
//	OPSKEEPER_EDGE_PLUGIN_<NAME>_ENDPOINT   = https://manager.example.com/loki/api/v1/push
//	OPSKEEPER_EDGE_PLUGIN_<NAME>_AUTH_USER  = <basic-auth user, typically = edge access key>
//	OPSKEEPER_EDGE_PLUGIN_<NAME>_AUTH_PASS  = <basic-auth pass, typically = edge secret key; bearer token when AUTH_USER empty>
//	OPSKEEPER_EDGE_PLUGIN_<NAME>_SPEC_JSON  = {"journald_units": ["sshd"], ...}
//
...
func (e *EnvConfigFetcher) Fetch(_ context.Context) (map[string]PluginConfig, error) {
	...
	for _, name := range e.knownPlugins {
		prefix := "OPSKEEPER_EDGE_PLUGIN_" + strings.ToUpper(name) + "_"
		cfg := PluginConfig{
			Enabled:  envBool(prefix + "ENABLED"),
			EdgeID:   edgeID,
			Endpoint: os.Getenv(prefix + "ENDPOINT"),
			AuthUser: firstNonEmpty(os.Getenv(prefix+"AUTH_USER"), os.Getenv("OPSKEEPER_EDGE_ACCESS_KEY")),
			AuthPass: firstNonEmpty(os.Getenv(prefix+"AUTH_PASS"), os.Getenv("OPSKEEPER_EDGE_SECRET_KEY")),
		}
