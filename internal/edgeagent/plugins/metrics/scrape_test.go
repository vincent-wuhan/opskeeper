...
// SourceLabel — push samples land without an opskeeper_source label so
...
func TestParseSpec_Defaults(t *testing.T) {
	...
	if got.SourceLabel != "" {
		t.Errorf("source label = %q; want empty (no opskeeper_source label on push)", got.SourceLabel)
	}
}
...
func TestPlugin_PushesSamplesWithEdgeID(t *testing.T) {
	...
	if c.Source != "" {
		t.Errorf("call Source = %q; want empty (default — no opskeeper_source label)", c.Source)
	}
	if len(c.Samples) == 0 {
