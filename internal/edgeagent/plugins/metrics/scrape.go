...
func parseSpec(spec map[string]interface{}) (specView, error) {
	...
	if spec == nil {
		...
		// `opskeeper_source` label, so push samples land looking identical
	...
	// push samples don't carry an opskeeper_source label (see comment in
...
// distinguishable in `opskeeper_source` queries.
