...
func TestInjector_Inject_SupportedType(t *testing.T) {
	...
	if res.Type != "kafka.kill_broker" {
		t.Errorf("Type = %q, want %q", res.Type, "kafka.kill_broker")
	}
...
func TestInjector_Inject_GeneratesUniqueIDs(t *testing.T) {
	i := New()
	r1, _ := i.Inject(context.Background(), injector.InjectSpec{Type: "kafka.kill_broker"})
	r2, _ := i.Inject(context.Background(), injector.InjectSpec{Type: "kafka.kill_broker"})
...
func TestInjector_Cleanup_HappyPath(t *testing.T) {
	i := New()
	res, _ := i.Inject(context.Background(), injector.InjectSpec{Type: "kafka.kill_broker"})
...
func TestInjector_AllSupportedTypes(t *testing.T) {
	...
	t.Logf("kafka: %d supported types all accepted by skeleton", len(types))
}
