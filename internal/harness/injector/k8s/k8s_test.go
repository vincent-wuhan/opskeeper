...
func TestInjector_Inject_SupportedType(t *testing.T) {
	...
	if res.Type != "k8s.set_bad_image" {
		t.Errorf("Type = %q, want %q", res.Type, "k8s.set_bad_image")
	}
...
func TestInjector_Inject_GeneratesUniqueIDs(t *testing.T) {
	i := New()
	r1, _ := i.Inject(context.Background(), injector.InjectSpec{Type: "k8s.set_bad_image"})
	r2, _ := i.Inject(context.Background(), injector.InjectSpec{Type: "k8s.set_bad_image"})
...
func TestInjector_Cleanup_HappyPath(t *testing.T) {
	i := New()
	res, _ := i.Inject(context.Background(), injector.InjectSpec{Type: "k8s.set_bad_image"})
...
func TestInjector_AllSupportedTypes(t *testing.T) {
	...
	t.Logf("k8s: %d supported types all accepted by skeleton", len(types))
}
