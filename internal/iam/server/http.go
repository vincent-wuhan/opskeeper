...
func (h *Handler) RegisterProtected(r chi.Router) {
	r.Post("/v1/agentteams/token", h.issueAgentTeamsToken)
...
type agentTeamsTokenReq struct {
...
type agentTeamsTokenResp struct {
...
// @Summary Issue a short-lived AgentTeams MCP service token
// @Router /v1/agentteams/token [post]
// @Success 200 {object} agentTeamsTokenResp
func (h *Handler) issueAgentTeamsToken(w http.ResponseWriter, r *http.Request) {
	w.Header().Set("X-Opskeeper-Version", "v1")
	if r.Header.Get("X-Opskeeper-Version") != "v1" {
		http.Error(w, "X-Opskeeper-Version must be v1", http.StatusBadRequest)
		return
	}
	if !requireAdmin(w, r) {
		return
	}
	var in agentTeamsTokenReq
	if err := decode(r, &in); err != nil {
		writeErr(w, err)
		return
	}
	caller, _ := tenantctx.From(r.Context())
	token, err := h.svc.IssueAgentTeamsToken(r.Context(), caller.Role, biz.AgentTeamsTokenRequest{
