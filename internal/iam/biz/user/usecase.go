...
type AgentTeamsTokenRequest struct {
	TenantID     string
	Worker       string
	Role         string
	AllowedTools []string
...
func (u *Usecase) IssueAgentTeamsToken(ctx context.Context, requesterRole string, request AgentTeamsTokenRequest) (*AgentTeamsToken, error) {
	if requesterRole != model.RoleAdmin {
		return nil, errs.ErrForbidden
	}
	request.TenantID = strings.TrimSpace(request.TenantID)
	request.Worker = strings.TrimSpace(request.Worker)
	request.Role = strings.TrimSpace(request.Role)
	if request.TenantID == "" || request.Worker == "" || request.Role == "" || len(request.AllowedTools) == 0 {
		return nil, fmt.Errorf("%w: tenant_id, worker, role, and allowed_tools are required", errs.ErrInvalid)
	}
	if !auth.AgentTeamsWorkerBoundToRole(request.Worker, request.Role) {
		return nil, fmt.Errorf("%w: worker must equal opskeeper-<role>", errs.ErrInvalid)
	}
	if request.TTLSeconds == 0 {
		request.TTLSeconds = 900
	}
	if request.TTLSeconds < 60 || request.TTLSeconds > 900 {
		return nil, fmt.Errorf("%w: ttl_seconds must be between 60 and 900", errs.ErrInvalid)
	}
	tools := make([]string, len(request.AllowedTools))
	for index, tool := range request.AllowedTools {
		tools[index] = strings.TrimSpace(tool)
		if !auth.AgentTeamsRoleAllows(request.Role, tools[index]) {
			return nil, fmt.Errorf("%w: tool %q is not allowed for role %q", errs.ErrInvalid, tools[index], request.Role)
		}
	}
	token, err := u.signer.SignAgentTeamsService(auth.AgentTeamsServiceClaims{
	...
	if err != nil {
		return nil, fmt.Errorf("sign agentteams token: %w", err)
	}
	return &AgentTeamsToken{Token: token, ExpiresIn: request.TTLSeconds}, nil
}
