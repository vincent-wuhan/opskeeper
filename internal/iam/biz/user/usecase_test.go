...
func TestIssueAgentTeamsToken_AdminOnlyAndRoleScoped(t *testing.T) {
	uc := newTestUsecase(t)
	ctx := context.Background()

	token, err := uc.IssueAgentTeamsToken(ctx, model.RoleUser, AgentTeamsTokenRequest{
		TenantID: "tenant-a", Worker: "opskeeper-investigator", Role: "investigator",
		AllowedTools: []string{"loop.investigate"}, TTLSeconds: 300,
	})
	if !errors.Is(err, errs.ErrForbidden) || token != nil {
		t.Fatalf("non-admin issuance = %+v, %v; want forbidden", token, err)
	}

	token, err = uc.IssueAgentTeamsToken(ctx, model.RoleAdmin, AgentTeamsTokenRequest{
		TenantID: "tenant-a", Worker: "opskeeper-investigator", Role: "investigator",
		AllowedTools: []string{"loop.investigate"}, TTLSeconds: 300,
	})
	if err != nil {
		t.Fatalf("admin issuance: %v", err)
	}
	claims, err := uc.verifyTokenForTest(token.Token)
	if err != nil {
		t.Fatalf("verify issuance: %v", err)
	}
	if claims.AgentTeams == nil || claims.AgentTeams.Worker != "opskeeper-investigator" ||
		claims.AgentTeams.Role != "investigator" || len(claims.AgentTeams.AllowedTools) != 1 {
		t.Fatalf("claims = %+v", claims)
	}
}

func TestIssueAgentTeamsToken_RejectsRoleEscalation(t *testing.T) {
	uc := newTestUsecase(t)
	_, err := uc.IssueAgentTeamsToken(context.Background(), model.RoleAdmin, AgentTeamsTokenRequest{
		TenantID: "tenant-a", Worker: "opskeeper-investigator", Role: "investigator",
		AllowedTools: []string{"recovery.execute"}, TTLSeconds: 300,
	})
	if err == nil {
		t.Fatal("investigator token with recovery.execute was issued")
	}
}

func TestIssueAgentTeamsToken_RejectsUnboundWorkerRole(t *testing.T) {
	uc := newTestUsecase(t)
	_, err := uc.IssueAgentTeamsToken(context.Background(), model.RoleAdmin, AgentTeamsTokenRequest{
		TenantID: "tenant-a", Worker: "generic", Role: "repairer",
		AllowedTools: []string{"recovery.execute"}, TTLSeconds: 300,
	})
	if err == nil {
		t.Fatal("unbound worker/role token was issued")
	}
}

func (u *Usecase) verifyTokenForTest(token string) (*auth.Claims, error) {
	return u.signer.Verify(token)
}
