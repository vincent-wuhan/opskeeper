...
_最近更新：2026-07-14 · English version: [`ROADMAP.md`](ROADMAP.md)_

开源仓 OpsKeeper 的工作路线图。把 2026-06-06 规划讨论的新方向与散落在
...
OpsKeeper 的护城河是 `geminio` 双向通道 + AI agent 能在客户机器上**真动手**。
...
- **A.3** `✓` 边端变更事件源
  - 当前 audit log 只看得到走 OpsKeeper 的变更
...
- **G.1.1** `□` 端口冲突预检
  ...
  - 同步传到 `OPSKEEPER_PUBLIC_URL`
- **G.1.2** `□` 共存 vs 独占问询
  - preflight 问："这台机是不是只跑 opskeeper"
  - 答案决定端口和 public URL
