...
// validates the Cookie). This step is REQUIRED, not optional: opskeeper's own
  ...
  try {
    ...
    for (const it of r.items) {
      if (it.key === 'root_url' && (it.value ?? '').trim() !== '') {
        ...
        // opskeeper's same-origin nginx /grafana/ proxy in that case.
...
async function buildGrafanaUrl(input: DrilldownInput): Promise<string | null> {
  ...
  const dashboardUid = grafanaDashboardUid.trim() || 'opskeeper-server-detail';
...
// uid (opskeeper-loki / opskeeper-tempo / opskeeper-prometheus). query carries
