...
const PRESET_DEFS: RulePresetDef[] = [
  ...
  },
  {
    ...
    hintZh: 'Prom 直接 scrape OpsKeeper manager 失败 ⇒ manager 进程挂了 / 网络断了。比 prom_write_total 可靠（不需要 remote_write 通畅）。',
    hintEn: "Prom scrape of the OpsKeeper manager fails — manager process down or network broken. More reliable than prom_write_total (doesn't depend on remote_write).",
    exprPreview: 'up{job="opskeeper-manager"} == 0',
    group: '平台健康',
    draftBase: {
      kind: 'metric_raw',
      severity: 'critical',
      scope_type: 'global',
      enabled: true,
      spec: { expr: 'up{job="opskeeper-manager"} == 0' },
    },
  },
  {
    id: 'edge_agent_down',
    ...
    exprPreview: 'up{job="opskeeper-edge"} == 0',
    group: '平台健康',
    draftBase: {
      kind: 'metric_raw',
      severity: 'critical',
      scope_type: 'global',
      enabled: true,
      spec: { expr: 'up{job="opskeeper-edge"} == 0' },
    },
  },
  {
    id: 'notify_failed_5m',
...

  {
    ...
    exprPreview: 'count_over_time({opskeeper_source=~"journald:.+"} |~ "(?i)panic|oom|fatal" [5m]) >= 1',
    group: '日志',
    draftBase: {
      kind: 'log_match',
      severity: 'critical',
