...
export default function EdgeDetailPage() {
  ...
  const refreshMetrics = useCallback(async () => {
    ...
    // Filter by host device_id only. The historical `opskeeper_source=""`
    ...
    // embedded push and carries `opskeeper_source="embedded"`, so the
  ...
  const promExprs = useMemo(() => {
    ...
    // Same fix as above — drop the opskeeper_source="" matcher because
    // every sample now carries opskeeper_source="embedded"; the legacy
...
function DatabaseMetricsSpecForm({
  ...
  return (
    <div className="space-y-3">
      ...
      {sources.length === 0 && (
        <div className="rounded-md border border-dashed border-zinc-800 px-3 py-3 text-[11px] text-zinc-500">
          {tr('用于需要 OpsKeeper 在 edge 上托管 exporter 的数据库。已有 /metrics 接口的数据库 exporter 请放到 custommetrics。', 'Use this when OpsKeeper should manage the exporter on the edge. Existing database /metrics exporters belong in custommetrics.')}
        </div>
      )}
      {sources.length > 0 && (
        <div className="overflow-hidden rounded-md border border-zinc-800 bg-zinc-950/40 divide-y divide-zinc-800/60">
  ...
  if (dbType === 'postgresql') {
    return (
      ...
      >
        ...
        <div className="grid gap-3 md:grid-cols-3">
          <SpecInput label="config_file" value={exporterString(exporter, 'config_file')} placeholder="/etc/opskeeper-edge/postgres_exporter.yml" onChange={(v) => setExporterField('config_file', v)} />
          <SpecInput label="extend_query_path" value={exporterString(exporter, 'extend_query_path')} placeholder="/etc/opskeeper-edge/postgres-queries.yaml" onChange={(v) => setExporterField('extend_query_path', v)} />
    ...
    return (
      ...
      >
        ...
        <div className="grid gap-3 md:grid-cols-3">
          ...
          <SpecInput label="script" value={exporterString(exporter, 'script')} placeholder="/etc/opskeeper-edge/redis-metrics.lua" onChange={(v) => setExporterField('script', v)} />
        </div>
        <div className="grid gap-3 md:grid-cols-2">
          ...
          <>
            <div className={cn('mt-3 grid gap-3', showTLSKeyFile ? 'md:grid-cols-3' : 'md:grid-cols-2')}>
              <SpecInput
                ...
                placeholder="/etc/opskeeper-edge/certs/ca.crt"
                onChange={(v) => setCredential('tls_ca_file', v)}
              />
              <SpecInput
                label="tls_cert_file"
                value={value('tls_cert_file')}
                placeholder={dbType === 'mongodb' ? '/etc/opskeeper-edge/certs/client.pem' : '/etc/opskeeper-edge/certs/client.crt'}
              ...
              {showTLSKeyFile && (
                <SpecInput
                  ...
                  placeholder="/etc/opskeeper-edge/certs/client.key"
    ...
    <div className="space-y-4">
      ...
      <div>
        ...
        {labelEntries.length === 0 && (
          <div className="rounded-md border border-dashed border-zinc-800 px-2 py-2 text-[11px] text-zinc-500">
            {tr('—（仅会自动注入设备 / opskeeper_source 等白名单 label）', '— (only allow-listed labels like device / opskeeper_source are auto-injected)')}
          </div>
        )}
        <div className="space-y-1.5">
