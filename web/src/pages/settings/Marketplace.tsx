...
//
// 注册表入口（opskeeper-official / openclaw-bridge）下架——后端 v1 stub 不
...
// 上规模再启动 opskeeper-official"）。后端 SourceTypeRegistry 保留，未来恢复
      ...
      <div className="rounded-md border border-zinc-800 bg-zinc-950/40 px-3 py-3">
        ...
        {tab === 'local' && (
          <Field
            ...
            hint={tr(
              'manager 主机上 admin 已 scp / 解压好的目录，例如 /var/lib/opskeeper/uploads/etcd-tools',
              'Directory on the manager host that an admin has already scp\'d / extracted, e.g. /var/lib/opskeeper/uploads/etcd-tools',
            )}
          >
            <input
              type="text"
              ...
              placeholder="/var/lib/opskeeper/uploads/..."
              disabled={!isAdmin || installing}
  ...
  switch (kind) {
    ...
    case 'invalid':
      if (/allow.*list|allow.list|allowed/i.test(apiMsg)) {
        return tr(
          '该 source 未在允许列表，开发模式 OPSKEEPER_MARKETPLACE_DEVMODE=true 可放开',
          'This source is not in the allow-list; set OPSKEEPER_MARKETPLACE_DEVMODE=true for dev mode',
        );
      }
      return apiMsg
