...
export default function SettingsWebshell() {
  ...
  const refresh = useCallback(async () => {
    ...
    try {
      ...
      // the auditor IS the actor (me.email matches own opskeeper_user_id).
  ...
  const visibleItems = useMemo(() => {
    ...
    return items.filter((s) => {
      ...
      if (!q) return true;
      const user = userMap.get(s.opskeeper_user_id);
      const userLabel = user
        ? (user.display_name || '') + ' ' + (user.email || '')
        : tr(`用户 #${s.opskeeper_user_id}`, `User #${s.opskeeper_user_id}`);
  ...
  return (
    <main className="anim-fade flex flex-1 flex-col overflow-hidden">
      <PageHeader
        ...
        subtitle={
          <span>
            {tr('当前活跃 ', 'Active ')}<span className="text-zinc-300">{counts.active}</span>{tr(' 个 · 历史 ', ' · history ')}<span className="text-zinc-300">{counts.history}</span>{tr(' 条', ' records')}
          </span>
        }
        actions={
