...
export default function AlertRulesPage() {
  ...
  const stats = useMemo(() => {
    ...
    for (const r of items) {
      ...
      if (r.source_type === 'opskeeper_builtin') builtin++;
              ...
              <p className="mt-0.5 text-xs text-zinc-500">
                ...
                {runtime && runtime.evaluator_interval_seconds > 0 && (
                  <span
                    ...
                    title={tr(
                      `evaluator 每 ${humanDur(runtime.evaluator_interval_seconds)} 跑一次全量规则;通知 cooldown ${humanDur(runtime.notify_cooldown_seconds)}。出厂默认 5min / 10min,可由 OPSKEEPER_ALERT_EVAL_INTERVAL / OPSKEEPER_ALERT_COOLDOWN 覆盖。`,
                      `Evaluator runs every ${humanDur(runtime.evaluator_interval_seconds)}; notification cooldown ${humanDur(runtime.notify_cooldown_seconds)}. Factory defaults 5min / 10min, override via OPSKEEPER_ALERT_EVAL_INTERVAL / OPSKEEPER_ALERT_COOLDOWN.`,
                    )}
                  >
                    ⏱ {tr(
...
function RuleRow({
  ...
  const { tr } = useI18n();
  const isBuiltin = rule.source_type === 'opskeeper_builtin';
    ...
    return (
      <div className="space-y-3">
        <Field label="stream_selector（LogQL label match）">
          <input
            value={(form.spec?.stream_selector as string) ?? '{opskeeper_source=~"journald(:.*)?"}'}
            onChange={(e) => setSpec({ stream_selector: e.target.value })}
            placeholder='{device_id="123",opskeeper_source=~"journald(:.*)?"}'
            className="w-full rounded-md border border-zinc-800 bg-zinc-950 px-2 py-1.5 font-mono text-xs text-zinc-100 focus:border-zinc-600 focus:outline-none"
          />
        </Field>
    ...
    return (
      <div className="space-y-3">
        <Field label="stream_selector">
          <input
            value={(form.spec?.stream_selector as string) ?? '{opskeeper_source=~".+"}'}
            onChange={(e) => setSpec({ stream_selector: e.target.value })}
