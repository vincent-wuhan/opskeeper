...
// into a single opskeeper-managed Grafana dashboard (uid opskeeper-monitor)
// asynchronously so deep-links keep working — see biz/grafana.Service.
// Sync is one-way (opskeeper → Grafana); edits made in Grafana are
    ...
    targets: [
      {
        ...
        // now carries opskeeper_source="embedded". The old empty-string
        // matcher (opskeeper_source="") would have dropped 100% of points.
    ...
    targets: [
      {
        ...
        // opskeeper_source="" matcher was removed).
    ...
    targets: [
      {
        ...
        // opskeeper_source are too noisy for a fleet panel.
...
export default function MonitorPage() {
  ...
  // The Monitor page mirrors its panels into the opskeeper-monitor dashboard
  // (biz/grafana.SyncMonitorPanels: core fleet panels + user panels), so
  // "在 Grafana 中打开" must target that — not the per-device
  // opskeeper-server-detail drill-down dashboard, which lacks these panels and
  // was why the newly-added panels didn't show up after jumping over.
  const dashboardUid = 'opskeeper-monitor';
  ...
  const handleDeletePanel = useCallback(
    (p: MonitorPanel) => {
      if (!window.confirm(tr(
        `确认删除面板 “${p.title}”？同步会从 Grafana opskeeper-monitor 仪表盘移除。`,
        `Delete panel “${p.title}”? It will also be removed from the Grafana opskeeper-monitor dashboard.`,
      ))) {
        return;
      }
          ...
          <section>
            <header className="mb-2 flex items-center gap-2">
              ...
              <span className="text-[11px] text-zinc-600">
                {tr(
                  `(${userPanels.length}) — 同步到 Grafana opskeeper-monitor 仪表盘`,
                  `(${userPanels.length}) — synced to the Grafana opskeeper-monitor dashboard`,
                )}
              </span>
            </header>
            <UserPanelGrid
