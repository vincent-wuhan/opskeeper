    ...
    >
      <form id="monitor-panel-form" onSubmit={handleSubmit} className="space-y-4">
        ...
        <p className="text-[11px] text-zinc-500">
          {tr('保存后会异步同步到 Grafana 的 opskeeper-monitor 仪表盘（单向：opskeeper → Grafana）。同步失败不影响本面板渲染。', 'After save, the panel is asynchronously synced to the Grafana opskeeper-monitor dashboard (one-way: opskeeper → Grafana). A sync failure does not affect rendering here.')}
        </p>
      </form>
    </Modal>
  );
