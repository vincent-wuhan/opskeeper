#!/usr/bin/env bash
# opskeeper install.sh - first-install or re-install on top of existing.
...
ensure_host_gateway_env() {
    ...
    if [[ -z "$gateway" ]]; then
        ...
        log_error "set OPSKEEPER_HOST_GATEWAY=<docker0 gateway IP> in ${ENV_FILE}, then re-run sudo ./install.sh"
        return 1
    fi
...
if [[ -d "$SCRIPT_DIR/edge" ]]; then
    ...
    if [[ -x "$INSTALL_DIR/edge/build-edge-bundle.sh" && -n "$_edge_ver" ]]; then
        ...
        # in the Makefile), so we glob the present opskeeper-edge-linux-* binaries
...
# (NFS / iSCSI / NVMe) by overriding OPSKEEPER_DATA_DIR and OPSKEEPER_LOG_DIR.
...
for v in \
    opskeeper_opskeeper_mysql_data opskeeper_mysql_data mysql_data \
    opskeeper_opskeeper_logs opskeeper_logs \
    opskeeper_prometheus_data prometheus_data \
    opskeeper_grafana_data grafana_data \
    opskeeper_loki_data loki_data \
    opskeeper_tempo_data tempo_data \
    opskeeper_qdrant_data qdrant_data; do
...
# ${OPSKEEPER_DATA_DIR:-...} into the bind paths at up time.
...
if [[ ! -f "$INSTALL_DIR/certs/tls.crt" || ! -f "$INSTALL_DIR/certs/tls.key" ]]; then
    log_info "generating self-signed TLS cert (valid 365d, CN=opskeeper)"
...
# ---------- load docker images ----------
# Both opskeeper (manager) and frontier (broker) images ship in the tarball.
...
if [[ -f "$SCRIPT_DIR/images/opskeeper.tar" ]]; then
    log_info "loading opskeeper image (docker load)"
    docker load -i "$SCRIPT_DIR/images/opskeeper.tar"
else
    log_warn "images/opskeeper.tar not found; assuming image already present"
fi
...
if [[ -f "$SCRIPT_DIR/images/opskeeper-web.tar" ]]; then
    log_info "loading opskeeper-web (frontend + nginx) image (docker load)"
...
if [[ -z "$VERSION_FROM_FILE" ]]; then
    VERSION_FROM_FILE=$(grep -E '^OPSKEEPER_VERSION=' "$SCRIPT_DIR/.env.example" | cut -d= -f2- | tr -d '[:space:]' || true)
fi
[[ -n "$VERSION_FROM_FILE" ]] || { log_error "cannot determine opskeeper version"; exit 1; }

if ! docker image inspect "opskeeper:${VERSION_FROM_FILE}" >/dev/null 2>&1; then
    log_error "docker image opskeeper:${VERSION_FROM_FILE} not found after load"
    exit 1
fi
log_info "opskeeper:${VERSION_FROM_FILE} image ready"
...
# OPSKEEPER_JWT_SECRET (64 chars)
if is_blank OPSKEEPER_JWT_SECRET; then
    fill_blank OPSKEEPER_JWT_SECRET "$(gen_secret 64)"
    log_info "generated OPSKEEPER_JWT_SECRET"
fi

# OPSKEEPER_ADMIN_PASSWORD (record it for the final banner)
if is_blank OPSKEEPER_ADMIN_PASSWORD; then
    GENERATED_ADMIN_PASSWORD=$(gen_secret 20)
    fill_blank OPSKEEPER_ADMIN_PASSWORD "$GENERATED_ADMIN_PASSWORD"
    ADMIN_PASSWORD_NEWLY_GENERATED=1
    log_info "generated OPSKEEPER_ADMIN_PASSWORD (printed once in final banner)"
fi
...
# OPSKEEPER_PUBLIC_URL — the address EDGES use to reach this manager's data
...
#   2. Exported env var OPSKEEPER_PUBLIC_URL                        → use (non-interactive automation)
...
if is_blank OPSKEEPER_PUBLIC_URL; then
    ...
    PORT_FOR_URL=$(grep -E '^OPSKEEPER_HTTP_PORT=' "$ENV_FILE" | cut -d= -f2- || echo 443)
    : "${PORT_FOR_URL:=443}"
    ...
    RESOLVED_PUBLIC_URL=""
    if [[ -n "${OPSKEEPER_PUBLIC_URL:-}" ]]; then
        RESOLVED_PUBLIC_URL="${OPSKEEPER_PUBLIC_URL}"
        log_info "OPSKEEPER_PUBLIC_URL taken from environment: ${RESOLVED_PUBLIC_URL}"
...
the detected default, or type the correct address." \
        ...
        log_warn "no interactive terminal and OPSKEEPER_PUBLIC_URL unset; auto-set to ${RESOLVED_PUBLIC_URL}"
        log_warn "if that is an INTERNAL address your remote edges can't reach, edit ${ENV_FILE} and restart"
    fi
    fill_blank OPSKEEPER_PUBLIC_URL "$RESOLVED_PUBLIC_URL"
    log_info "OPSKEEPER_PUBLIC_URL=${RESOLVED_PUBLIC_URL} (edit .env to change)"
fi

# Bump OPSKEEPER_VERSION to match VERSION file.
sed -i.bak -E "s|^OPSKEEPER_VERSION=.*|OPSKEEPER_VERSION=${VERSION_FROM_FILE}|" "$ENV_FILE"
rm -f "${ENV_FILE}.bak"
chmod 600 "$ENV_FILE"
...
OPSKEEPER_HTTP_PORT=$(grep -E '^OPSKEEPER_HTTP_PORT=' "$ENV_FILE" | cut -d= -f2- || true)
OPSKEEPER_HTTP_REDIRECT_PORT=$(grep -E '^OPSKEEPER_HTTP_REDIRECT_PORT=' "$ENV_FILE" | cut -d= -f2- || true)
OPSKEEPER_TUNNEL_PORT=$(grep -E '^OPSKEEPER_TUNNEL_PORT=' "$ENV_FILE" | cut -d= -f2- || true)
PROM_PORT=$(grep -E '^PROM_PORT=' "$ENV_FILE" | cut -d= -f2- || true)
ADMIN_EMAIL=$(grep -E '^OPSKEEPER_ADMIN_EMAIL=' "$ENV_FILE" | cut -d= -f2- || true)
...
# overrides (OPSKEEPER_INVESTIGATOR_ENABLED, custom feature flags).
...
# ---------- wait for /healthz ----------
# nginx terminates TLS on host port ${OPSKEEPER_HTTP_PORT} (443 by default) and
# proxies /healthz to the manager. -k tolerates the self-signed cert.
log_info "waiting for /healthz on https://localhost:${OPSKEEPER_HTTP_PORT} (up to 60s)"
HEALTH_OK=0
for i in $(seq 1 30); do
    if curl -fsSk "https://localhost:${OPSKEEPER_HTTP_PORT}/healthz" >/dev/null 2>&1; then
        HEALTH_OK=1
        log_info "opskeeper is healthy (took ~$((i*2))s)"
        break
    fi
    printf '.'
    sleep 2
...
if [[ $HEALTH_OK -eq 0 ]]; then
    log_warn "opskeeper did not become healthy within 60s"
    log_warn "check logs: docker compose -f $INSTALL_DIR/docker-compose.yml logs opskeeper"
...
# .env but does NOT export to the current shell, so reading `$OPSKEEPER_PUBLIC_URL`
# would come back empty. Read the file. Fall back to hostname / route only
# when .env genuinely has no value.
HOST_FROM_ENV="$(grep -E '^OPSKEEPER_PUBLIC_URL=' "$ENV_FILE" 2>/dev/null | cut -d= -f2- | sed -E 's#^[a-zA-Z]+://##; s#[:/].*$##')"
...
echo "${C_BOLD}${C_GREEN}  opskeeper installation complete${C_RESET}"
echo "${C_BOLD}${C_CYAN}===============================================================${C_RESET}"
echo ""
...
if [[ "$OPSKEEPER_HTTP_PORT" == "443" ]]; then
    WEB_URL="https://${HOST_HINT}/"
    API_URL="https://${HOST_HINT}/api/v1"
else
    WEB_URL="https://${HOST_HINT}:${OPSKEEPER_HTTP_PORT}/"
    API_URL="https://${HOST_HINT}:${OPSKEEPER_HTTP_PORT}/api/v1"
fi
...
echo "${C_BOLD}Tunnel endpoint:${C_RESET} ${HOST_HINT}:${OPSKEEPER_TUNNEL_PORT}   (for edges)"
if [[ $PROFILE_MONITORING -eq 1 ]]; then
...
echo "       sudo docker compose -f $INSTALL_DIR/docker-compose.yml logs -f opskeeper"
echo "       sudo docker compose -f $INSTALL_DIR/docker-compose.yml restart opskeeper"
echo ""
echo "${C_BOLD}${C_CYAN}===============================================================${C_RESET}"
