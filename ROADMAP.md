...
_Last updated: 2026-07-14 · Chinese version: [`ROADMAP.zh-CN.md`](ROADMAP.zh-CN.md)_

Working roadmap for the open-core OpsKeeper project. Merges user-driven
...
OpsKeeper's moat is the `geminio` two-way tunnel plus an AI agent that can
...
- **A.3** `✓` Edge-side change watcher
  - today's audit log only sees OpsKeeper-mediated changes
...
- **B.2** `☑` Network probes as first-class BaseTools [archived 2026-07-14, b2b3-diag-tools] [v1.0]
  - `probe_tcp` / `probe_http` / `probe_dns`
  - `traceroute` / `mtr`
  - first-class output schemas so results render cleanly in the timeline
- **B.3** `☑` File, log, kernel diagnostics [archived 2026-07-14, b2b3-diag-tools] [v1.0]
  - `tail_file`, `grep_file`
...
- **D.1** `✓` Specialist sub-agents [v1.0]
...
- **F.4.2** `✓` DingTalk — completes the CN big three [v1.0]
- **F.4.3** `□` Microsoft Teams — overseas enterprise
- **F.4.4** `◐` WeCom (企业微信) two-way [v1.0]
  - webhook-only today
...
- **G.1.1** `□` Port-conflict preflight
  ...
  - propagate to `OPSKEEPER_PUBLIC_URL`
...
- **I.1** `☑` SSO / SAML / OIDC [archived 2026-07-14, i1-sso; SAML P2 follow-up]
