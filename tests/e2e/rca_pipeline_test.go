...
//
//   - Start manager with OPSKEEPER_ALERT_EVAL_INTERVAL=2s (E1 pattern) +
//     OPSKEEPER_INVESTIGATOR_ENABLED=true (default OFF — gates the entire
//     investigator usecase, see cmd/opskeeper/main.go:1424).
...
func TestRCA_InvestigationPipeline_F1(t *testing.T) {
	env := testenv.Start(t,
		testenv.WithEnv("OPSKEEPER_ALERT_EVAL_INTERVAL", "2s"),
		testenv.WithEnv("OPSKEEPER_ALERT_ENABLED", "true"),
		...
		testenv.WithEnv("OPSKEEPER_INVESTIGATOR_ENABLED", "true"),
	)
	pair := env.LoginAdmin()
