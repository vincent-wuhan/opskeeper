#!/usr/bin/env bash
# install.sh — opskeeper-teamharness QwenPaw adapter installer
#
# Builds QwenPaw plugin package from base + adapter, then calls
# `qwenpaw plugin install --force`.
#
# Reuses AgentTeams teamharness/adapters/qwenpaw/scripts/build-qwenpaw-plugin.rb
# pattern but for opskeeper-teamharness.
...
stage_dir="$(mktemp -d "${TMPDIR:-/tmp}/opskeeper-qwenpaw-install.XXXXXX")"
cleanup() { rm -rf "$stage_dir"; }
trap cleanup EXIT

package_zip="$(OUT_DIR="${stage_dir}/dist" ruby "${BUILD_SCRIPT}" "${PLUGIN_DIR}/plugin.yaml" | tail -n 1)"
